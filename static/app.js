...
function collectData() {
  ...
  // 拓扑图片数据（双重读取：单独key优先，fallback到topology_data内嵌的image）
  let topologyImage = localStorage.getItem("topology_image") || null;
  if (!topologyImage) {
    const topoData = localStorage.getItem("topology_data");
    if (topoData) {
      try {
        const parsed = JSON.parse(topoData);
        if (parsed.image && parsed.image.startsWith("data:image/png;base64,")) {
          topologyImage = parsed.image;
          console.log('[collectData] 从 topology_data.image 恢复拓扑截图');
        } else if (parsed.nodes && parsed.nodes.length > 0) {
          console.warn('[collectData] 有拓扑数据但无截图，请到拓扑编辑器重新保存');
        }
      } catch(e) {}
    }
  }
  return { ...data, ...content, quotes, topologyImage };
}
...
function renderSummaries() {
  ...
  if (aEl) {
    let topoImg = localStorage.getItem("topology_image");
    let topoData = localStorage.getItem("topology_data");
    // 如果单独 key 没有图片，尝试从 topology_data.image 恢复
    if (!topoImg && topoData) {
      try {
        const parsed = JSON.parse(topoData);
        if (parsed.image && parsed.image.startsWith("data:image/png;base64,")) {
          topoImg = parsed.image;
        }
      } catch(e) {}
    }
    let topoHtml = '';
    if (topoImg) {
      topoHtml = '<div style="margin-bottom:12px;border:1px solid #e8e8e8;border-radius:8px;overflow:hidden;background:#fafafa;">' +
        '<div style="padding:6px 12px;background:#f0f0f0;font-size:12px;color:#666;">已保存的拓扑图预览</div>' +
        '<img src="' + topoImg + '" style="width:100%;display:block;" alt="网络拓扑图"/>' +
        '</div>';
    }
    aEl.innerHTML = topoHtml + content.architectureRows.map(r => `<section><strong>${h(r.layer)}</strong><span>${h(r.product)}</span><p>${h(r.goal)}</p></section>`).join("");
    // 更新拓扑状态
    updateTopologyStatus(topoImg, topoData);
  }
...
function updateTopologyStatus(topoImg, topoData) {
  var statusEl = document.querySelector("#topologyStatus");
  if (!statusEl) return;
  if (!topoData) {
    statusEl.textContent = '⚠️ 尚未保存拓扑';
    statusEl.style.color = '#999';
  } else if (!topoImg) {
    statusEl.textContent = '⚠️ 拓扑已保存但缺少图片';
    statusEl.style.color = '#e6a817';
  } else {
    var imgSize = Math.round(topoImg.length / 1024);
    var nodes = 0;
    try { nodes = JSON.parse(topoData).nodes.length; } catch(e) {}
    statusEl.textContent = '✅ 已保存 (' + nodes + '个节点, 图片' + imgSize + 'KB)';
    statusEl.style.color = '#52c41a';
  }
}

// 手动刷新拓扑预览（供架构 Tab 按钮使用）
window.refreshTopologyPreview = function() {
  console.log('[拓扑刷新] 检查 localStorage...');
  var img = localStorage.getItem("topology_image");
  var data = localStorage.getItem("topology_data");
  console.log('[拓扑刷新] topology_image:', img ? '存在(' + Math.round(img.length/1024) + 'KB)' : '不存在');
  console.log('[拓扑刷新] topology_data:', data ? '存在' : '不存在');
  updateTopologyStatus(img, data);
  renderSummaries();
  renderPreview();
};
...
function renderPreview() {
  ...
  document.querySelector("#preview").innerHTML = `<style>${templateStyleCSS()}</style><div class="page">
    ...
    <section class="chapter">
      ...
      ${data.topologyImage ? `<div style="margin:12px 0;text-align:center;"><img src="${data.topologyImage}" style="max-width:100%;max-height:350px;border:1px solid #e8e8e8;border-radius:6px;" alt="网络拓扑图"/><p style="font-size:11px;color:#999;margin:4px 0 0;">图1：网络拓扑架构图</p></div>` : ""}
...
document.querySelector("#generateBtn")?.addEventListener("click", async () => {
  ...
  try {
    ...
    console.log('[生成Word] topologyImage:', cd.topologyImage ? '存在(' + Math.round(cd.topologyImage.length/1024) + 'KB)' : '缺失');
    const res = await fetch("/api/generate", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(cd) });
    if (!res.ok) throw new Error("生成失败");
    const result = await res.json();
    showToast("Word 已生成，正在下载");

    // 记录导出事件
    const scenarioIds = [...selectedScenarios];
    const firstScenario = scenarioIds.length > 0 ? scenarioIds[0] : "";
    recordExport(
      cd.solutionTitle,
      firstScenario,
      activeTemplateName,
      cd.products ? cd.products.length : 0,
      result.filename
    );
...
window.openTopologyEditor = function() {
  console.log('打开拓扑编辑器');
...
function getSessionId() {
  let sid = localStorage.getItem("stats_session_id");
  if (!sid) {
...
async function recordVisit() {
  const payload = {
    page: window.location.pathname === "/topology.html" || window.location.pathname.endsWith("/topology.html") ? "topology" : "index",
  ...
  try {
    ...
    visits.push({ ...payload, timestamp: Date.now() });
    // 只保留最近 500 条
...
async function recordExport(title, scenario, template, productCount, filename) {
  const payload = { title, scenario, template, productCount, filename };
  try {
    await fetch("/api/stats/export-event", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  try {
    ...
    exports.push({ ...payload, timestamp: Date.now() });
    if (exports.length > 500) exports.splice(0, exports.length - 500);
    localStorage.setItem("stats_exports", JSON.stringify(exports));
  } catch {}
}
