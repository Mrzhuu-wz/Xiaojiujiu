...
const DEFAULT_TEMPLATE = {
  ...
  styles: {
    ...
    CoverTitle:    { fontSize: 46, bold: true, color: "C00000", alignment: "center", spaceAfter: 120 },
...
function topologyImageRun(width = 5029200, height = 5249114) {
  return `<w:p><w:pPr><w:jc w:val="center"/></w:pPr><w:r><w:drawing><wp:inline xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" distT="0" distB="0" distL="0" distR="0"><wp:extent cx="${width}" cy="${height}"/><wp:effectExtent l="0" t="0" r="0" b="0"/><wp:docPr id="2" name="网络拓扑图"/><wp:cNvGraphicFramePr><a:graphicFrameLocks xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" noChangeAspect="1"/></wp:cNvGraphicFramePr><a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:nvPicPr><pic:cNvPr id="0" name="topology.png"/><pic:cNvPicPr/></pic:nvPicPr><pic:blipFill><a:blip r:embed="rIdTopology"/><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="${width}" cy="${height}"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr></pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>`;
}
...
function cover(data) {
  ...
  // 封面独立节：垂直居中 + 页眉Logo + 略微靠上
  // 分节符必须放在段落 w:pPr 内才能正确创建分页
  parts.push(`<w:p><w:pPr><w:sectPr><w:headerReference w:type="default" r:id="rIdHeader1"/><w:vAlign w:val="center"/><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="360" w:right="1440" w:bottom="2880" w:left="1440" w:header="720" w:footer="720" w:gutter="0"/></w:sectPr></w:pPr></w:p>`);
  return parts.join("");
}
...
function architectureSection(data) {
  ...
  const parts = [heading("二、方案架构"), paragraph(data.architecture || "")];
  // 如果有拓扑图，插入到表格上方
  if (data.topologyImage && data.topologyImage.startsWith("data:image/png;base64,")) {
    parts.push(topologyImageRun());
    parts.push(paragraph("图1：网络拓扑架构图", "Normal", "center"));
  }
  parts.push(table(rows, [1900, 2300, 4800]));
  return parts.join("");
}
...
async function buildDocx(data) {
  ...
  const topoImgB64 = data.topologyImage || "";
  ...
  if (topoImgB64 && topoImgB64.startsWith("data:image/png;base64,")) {
    docRelsParts.push('  <Relationship Id="rIdTopology" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/topology.png"/>');
  }
  docRelsParts.push('</Relationships>');

  zip.file("word/_rels/document.xml.rels", docRelsParts.join("\n"));
  zip.file("word/document.xml", documentXml(data));
  zip.file("word/styles.xml", buildStylesXml(template));
  zip.file("word/header1.xml", `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><w:p><w:pPr><w:jc w:val="right"/></w:pPr>${logoRun(1828800, 443300)}</w:p></w:hdr>`);
  ...
  // 写入拓扑图（如果有）
  if (topoImgB64 && topoImgB64.startsWith("data:image/png;base64,")) {
    const imgBuffer = Buffer.from(topoImgB64.split(",")[1], "base64");
    zip.file("word/media/topology.png", imgBuffer);
  }
